...
SELECT role_id, role_name, auths FROM roles
...
func (q *Queries) GetRoles(ctx context.Context) ([]Role, error) {
	...
	for rows.Next() {
		var i Role
		if err := rows.Scan(&i.RoleID, &i.RoleName, &i.Auths); err != nil {
			return nil, err
		}
